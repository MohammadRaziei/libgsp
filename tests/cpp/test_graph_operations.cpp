...
// Test clone method (returns value)
TEST_F(GraphOperationsTest, CloneMethod) {
    ...
    ASSERT_EQ(cloned_dense.numNodes(), original_dense->numNodes());
    ...
    ASSERT_EQ(cloned_sparse.numNodes(), original_sparse->numNodes());
...
TEST_F(GraphOperationsTest, ToSparseMethod) {
    ...
    ASSERT_EQ(sparse_to_sparse.numNodes(), original_sparse->numNodes());
...
TEST_F(GraphOperationsTest, ToDenseMethod) {
    ...
    ASSERT_EQ(dense_to_dense.numNodes(), original_dense->numNodes());
...
TEST_F(GraphOperationsTest, MixedOperationsWithSmartPointers) {
    ...
    // Clone it
    auto cloned = dense_ptr->clone();

    // Convert to sparse
    auto sparse = dense_ptr->toSparse();

    // Convert cloned to dense (no-op)
    auto dense_again = cloned.toDense();

    // Verify all are independent
    dense_ptr->setCoord(0, gsp::Coord(10.0, 10.0, 10.0));

    EXPECT_DOUBLE_EQ(dense_ptr->coord(0).x(), 10.0);
    EXPECT_DOUBLE_EQ(cloned.coord(0).x(), 1.0);  // Unchanged
    EXPECT_DOUBLE_EQ(sparse.coord(0).x(), 1.0);  // Unchanged
    EXPECT_DOUBLE_EQ(dense_again.coord(0).x(), 1.0);  // Unchanged

    // Verify names are preserved
    EXPECT_EQ(dense_ptr->name(0), "ptr0");
    EXPECT_EQ(cloned.name(0), "ptr0");
    EXPECT_EQ(sparse.name(0), "ptr0");
    EXPECT_EQ(dense_again.name(0), "ptr0");
}
...
TEST_F(GraphOperationsTest, ThresholdFunctionality) {
    ...
    auto sparse_threshold = dense_graph.toSparse(0.5);
    
    // Check that only edges with weight >= 0.5 are included
    auto sparse_edges = sparse_threshold.edges();
    
    // Should have edges: (0,1) with weight 0.8 and (2,3) with weight 0.6
    EXPECT_EQ(sparse_edges.size(), 2);
    
    // Verify the edges
    bool found_edge_0_1 = false;
    bool found_edge_2_3 = false;
    
    for (const auto& edge : sparse_edges) {
        if (edge.source() == 0 && edge.target() == 1) {
            EXPECT_DOUBLE_EQ(edge.weight(), 0.8);
            found_edge_0_1 = true;
        } else if (edge.source() == 2 && edge.target() == 3) {
            EXPECT_DOUBLE_EQ(edge.weight(), 0.6);
            found_edge_2_3 = true;
        }
    }
    
    EXPECT_TRUE(found_edge_0_1);
    EXPECT_TRUE(found_edge_2_3);
    
    // Test toDense with threshold 0.4 (should exclude edges with weight < 0.4)
    auto dense_threshold = sparse_threshold.toDense(0.4);
    
    // Since sparse_threshold already filtered edges < 0.5, and we're applying threshold 0.4,
    // all edges should remain
    auto dense_edges = dense_threshold.edges();
    EXPECT_EQ(dense_edges.size(), 2);
    
    // Test with zero threshold (should include all edges)
    auto sparse_all = dense_graph.toSparse(0.0);
    EXPECT_EQ(sparse_all.edges().size(), 4);
    
    // Test with high threshold (should exclude all edges)
    auto sparse_none = dense_graph.toSparse(1.0);
    EXPECT_EQ(sparse_none.edges().size(), 0);
}
